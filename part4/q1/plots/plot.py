...
type_to_label = {
    "t_1_c_1": "Thread: 1 - Core: 1",
    "t_1_c_2": "Thread: 1 - Core: 2",
    "t_2_c_1": "Thread: 2 - Core: 1",
    "t_2_c_2": "Thread: 2 - Core: 2"
}
markers = ["o", "v", "s", "*"]
runs = 3
...
for marker_idx, type in enumerate(thread_core_counts):
    ...
    ax.errorbar(x=file_df["QPS Mean"], y=file_df["p95 Mean"], xerr=file_df["QPS STD"], yerr=file_df["p95 SEM"], label=type_to_label[type], marker=markers[marker_idx], markersize=8, capsize=2)

ax.set_xlabel("Mean Queries per Second (QPS)", fontsize=18, labelpad=10, fontweight="semibold")
ax.set_ylabel("95th Percentile Latency (ms)", fontsize=18, fontweight="semibold")
...
ax.set_xticks(range(0, 125001, 5000))
